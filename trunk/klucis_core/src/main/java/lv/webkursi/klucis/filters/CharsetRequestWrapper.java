...
public final class CharsetRequestWrapper extends HttpServletRequestWrapper {
	...
	private Log log = LogFactory.getLog(CharsetRequestWrapper.class);
	...
	public Map getParameterMap() {		
		...
		for (Iterator i = oldMap.keySet().iterator(); i.hasNext(); ) {
			...
			log.info("paramValue[0] = '" + paramValue[0] + "'(" + paramValue[0].length() + ")");
			newMap.put(paramName, normalizeValues(paramName,paramValue));
			log.info("normalizedParamValue[0] = '" + normalizeValues(paramName,paramValue)[0] + "'(" + normalizeValues(paramName,paramValue)[0].length() + ")");
		}
		return newMap;
	}
